...
@tepthon.ar_cmd(pattern="كروباتي$")
async def oeo(event):
    result = await tepthon(functions.channels.GetGroupsForDiscussionRequest())
    alist = []
    for item in result.chats:
        ...
        alist.append(roz)
    if alist:
        await tepthon.send_message("me", "\n".join(alist))


@tepthon.ar_cmd(pattern="الحاظرهم$")
async def main(event):
    result = await tepthon(functions.contacts.GetBlockedRequest(offset=0, limit=1000000))
    alist = []
...
@tepthon.ar_cmd(pattern="قيد (.*)")
async def se(event):
    exe = event.text[5:]
    try:
        result = await tepthon(
...
@tepthon.ar_cmd(pattern="نوعه (.*)")
async def se(event):
    exe = event.text[5:]
    x = await tepthon.get_entity(exe)
...
@tepthon.ar_cmd(pattern="احذف (.*)")
async def se(event):
    exe = event.text[5:]
    await tepthon.get_dialogs()
    chat = exe
    await tepthon.delete_dialog(chat, revoke=True)
