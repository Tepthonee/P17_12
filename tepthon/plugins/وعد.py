...
@tepthon.ar_cmd(pattern="تجميع$")
async def _(event):
    await event.edit("حسنا, تأكد من انك مشترك ب قنوات الاشتراك الاجباري لتجنب الأخطأء")
    channel_entity = await tepthon.get_entity("@t06bot")
    await tepthon.send_message("@t06bot", "/start")
    await asyncio.sleep(10)
    msg0 = await tepthon.get_messages("@t06bot", limit=1)
    await msg0[0].click(2)
    await asyncio.sleep(10)
    msg1 = await tepthon.get_messages("@t06bot", limit=1)
    ...
    for i in range(100):
        ...
        try:
            ...
            msg2 = await tepthon.get_messages("@t06bot", limit=1)
...
# t.me/r0r77
@tepthon.ar_cmd(pattern="بخشيش وعد (.*)")
async def baqshis(event):
    for i in range(int("".join(event.text.split(maxsplit=2)[2:]).split(" ", 2)[0])):
        chat = event.chat_id
        await tepthon.send_message(chat, "بخشيش")
        await asyncio.sleep(605)


@tepthon.ar_cmd(pattern="راتب وعد (.*)")
async def ratb(event):
    for i in range(int("".join(event.text.split(maxsplit=2)[2:]).split(" ", 2)[0])):
        chat = event.chat_id
        await tepthon.send_message(chat, "راتب")
        await asyncio.sleep(605)


# none
@tepthon.ar_cmd(pattern="كلمات وعد (.*)")
async def waorwaad(event):
    for i in range(int("".join(event.text.split(maxsplit=2)[2:]).split(" ", 2)[0])):
        chat = event.chat_id
        await tepthon.send_message(chat, "كلمات")
        await asyncio.sleep(0.5)
        masg = await tepthon.get_messages(chat, limit=1)
        masg = masg[0].message
...
@tepthon.ar_cmd(pattern="استثمار وعد (.*)")
async def _(event):
    await event.edit(
    ...
    for i in range(int("".join(event.text.split(maxsplit=2)[2:]).split(" ", 2)[0])):
        chat = event.chat_id
        await tepthon.send_message(chat, "فلوسي")
        await asyncio.sleep(0.5)
        masg = await tepthon.get_messages(chat, limit=1)
        masg = masg[0].message
        ...
        if int(msg) > 500000000:
            await tepthon.send_message(chat, f"استثمار {msg}")
            await asyncio.sleep(10)
            mssag2 = await tepthon.get_messages(chat, limit=1)
