...
async def participant_check(channel, user_id):
    try:
        await tepthon(GetParticipantRequest(channel, int(user_id)))
        return True
...
async def fsub(event):
    ...
    if not str(channel).startswith("@"):
        ...
        if not await participant_check(channel, tepthon.uid):
            return await event.reply(
...
@tepthon.ar_cmd(pattern="تعطيل الاجباري")
