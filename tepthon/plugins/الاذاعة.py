...
@tepthon.ar_cmd(pattern=f"للكل(?: |$)(.*)", groups_only=True)
async def malath(event):
    ...
    is_admin = False
    try:
        await tepthon(GetParticipantRequest(event.chat_id, event.sender_id))
...
@tepthon.ar_cmd(pattern="ايقاف للكل", groups_only=True)
...
async def pmto(event):
    ...
    for user in users:
        ...
        if zilzal.text:
            try:
                user_entity = await tepthon.get_entity(user)
                if user_entity.bot or user_entity.deleted:
                    continue
                await tepthon.send_message(user_entity.id, zilzal, link_preview=False)
                success += 1
