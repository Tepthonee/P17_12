...
async def ban_user(chat_id, i, rights):
    try:
        await tepthon(functions.channels.EditBannedRequest(chat_id, i, rights))
        return True, None
    except Exception as exc:
        return False, str(exc)


@tepthon.ar_cmd(pattern="بوتي$")
async def _(event):
    TG_BOT_USERNAME = Config.TG_BOT_USERNAME
    await event.reply(f"**❃ البوت الخاص بك هو** \n {TG_BOT_USERNAME}")


@tepthon.ar_cmd(pattern="حالتي$")
async def _(event):
    text = "/start"
...
@tepthon.on(events.NewMessage(outgoing=False, pattern="/dev"))
async def _(event):
