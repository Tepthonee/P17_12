...
async def delpack(jmthonevent, conv, args, packname):
    try:
        ...
        await tepthon(unblock("stickers"))
        await conv.send_message("/delpack")
...
@tepthon.ar_cmd(pattern="ملصق(?:\s|$)([\s\S]*)")
async def kang(args):
    photo = None
    emojibypass = False
    is_anim = False
...
@tepthon.ar_cmd(pattern="حزمة(?:\s|$)([\s\S]*)")
...
@tepthon.ar_cmd(pattern="تحويل بملصق(?:\s|$)([\s\S]*)")
...
@tepthon.ar_cmd(pattern="معلومات_ألملصق$")
