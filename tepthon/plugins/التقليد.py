...
@tepthon.ar_cmd(pattern="تقليد$")
async def echo(event):
    if event.reply_to_msg_id is None:
        return await edit_or_reply(event, "⌔∮ يرجى الرد على الشخص الذي تريد ازعاجه ،")
    tepthonevent = await edit_or_reply(event, "⌔∮ يتم تفعيل هذا الامر انتظر قليلا ،")
    user, rank = await get_user_from_event(event, tepthonevent, nogroup=True)
    if not user:
        return
    ...
    try:
        ...
        await edit_delete(tepthonevent, f"⌔∮ خطأ\n`{str(e)}`")
    else:
        await edit_or_reply(
            tepthonevent,
...
@tepthon.ar_cmd(pattern="الغاء تقليد$")
async def echo(event):
    if event.reply_to_msg_id is None:
...
@tepthon.ar_cmd(pattern="حذف المقلدهم( للكل)?")
async def echo(event):
...
@tepthon.ar_cmd(pattern="المقلدهم( للكل)?$")
async def echo(event):
    input_str = event.pattern_match.group(1)
    private_chats = ""
...
@tepthon.ar_cmd(incoming=True, edited=False)
