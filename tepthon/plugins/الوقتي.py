...
from . import edit_delete, tepthon, logging
...
async def digitalpicloop():
    ...
    while DIGITALPICSTART:
        ...
        file = await tepthon.upload_file(autophoto_path)
        try:
            if i > 0:
                await tepthon(
                    functions.photos.DeletePhotosRequest(
                        await tepthon.get_profile_photos("me", limit=1)
                    )
                )
            i += 1
            await tepthon(functions.photos.UploadProfilePhotoRequest(file))
...
async def autoname_loop():
    while AUTONAMESTART := gvarstatus("autoname") == "true":
        ...
        LOGS.info(name)
        try:
            await tepthon(functions.account.UpdateProfileRequest(first_name=name))
...
async def autobio_loop():
    while AUTOBIOSTART := gvarstatus("autobio") == "true":
        ...
        LOGS.info(bio)
        try:
            await tepthon(functions.account.UpdateProfileRequest(about=bio))
        except FloodWaitError as ex:
            LOGS.warning(str(ex))
            await asyncio.sleep(ex.seconds)
        await asyncio.sleep(CHANGE_TIME)
...
@tepthon.ar_cmd(pattern="الصورة الوقتية$")
async def _(event):
    downloader = SmartDL(digitalpfp, digitalpic_path, progress_bar=False)
    downloader.start(blocking=False)
    while not downloader.isFinished():
...
@tepthon.ar_cmd(pattern="اسم وقتي$")
async def _(event):
...
@tepthon.ar_cmd(pattern="بايو وقتي$")
async def _(event):
...
@tepthon.ar_cmd(pattern="انهاء ([\s\S]*)")
async def _(event):
...
tepthon.loop.create_task(digitalpicloop())
tepthon.loop.create_task(autoname_loop())
tepthon.loop.create_task(autobio_loop())
